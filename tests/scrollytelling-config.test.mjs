...
test("必填键、重复键、非法尺寸、颜色和时间顺序给出具体错误", () => {
  for (const key of ["layout.worldWidth", "timeline.HALO_ENTER", "slot.halo-hero.name", "slot.halo-hero.src", "station.SR-2.displayId", "station.SR-2.label", "motion.chapterTitleY", "motion.chapterTitleDuration", "motion.chapterTitleViewportX", "motion.flashEnabled", "motion.irisOpacity"]) {
...
test("scene 状态可以正反向纯计算，milestone 和世界几何保持一致", () => {
  ...
  assert.equal(config.stations.length, 7);
  const stationGaps = config.stations.slice(1).map((station, index) => station.worldX - config.stations[index].worldX);
  ...
  assert.deepEqual(titles.map(title => title.name), config.stations.slice(1).map(station => station.label));
